...
test('database dump arguments never expose the password or request unnecessary routine/event privileges', () => {
  ...
  assert.ok(args.includes('--ssl-mode=REQUIRED'));
...
test('project backup snapshots database marker, env, package metadata, extensions and local files', async () => {
  ...
  try {
    ...
    assert.equal(result.manifest.project.extensions, true);
    assert.equal(result.manifest.project.localFiles, true);
...
test('restore resets database first and restores the exact project snapshot', async () => {
  ...
  try {
    ...
    await writeFile(join(cwd, 'package.json'), '{"dependencies":{"@yunsoft/yuncms":"0.1.1"}}\n');
    await writeFile(join(cwd, 'package-lock.json'), '{"lockfileVersion":3}\n');
    await writeFile(join(cwd, 'extensions', 'example.js'), 'old-extension');
    await writeFile(join(cwd, '.yuncms', 'uploads', 'asset.txt'), 'old-asset');
    ...
    await writeFile(join(cwd, 'extensions', 'example.js'), 'new-extension');
    await writeFile(join(cwd, '.yuncms', 'uploads', 'asset.txt'), 'new-asset');
    ...
    assert.match(await readFile(join(cwd, '.env'), 'utf8'), /DB_DATABASE/);
  } finally {
    await rm(cwd, { recursive: true, force: true });
  }
});
...
test('database reset disables FK checks and drops views before tables', async () => {
  ...
  const connection = {
    async query(sql, params = []) {
      ...
      if (normalized.startsWith('SELECT table_name, table_type')) {
        return [[
          { table_name: 'report_view', table_type: 'VIEW' },
  ...
  await resetDatabaseObjects({
    config: { database: 'yuncms_test' },
    createPool: () => pool,
    async closePool() { closed = true; },
  });

  assert.equal(calls[1].sql, 'SET FOREIGN_KEY_CHECKS = 0');
  assert.equal(calls[2].sql, 'DROP VIEW IF EXISTS `report_view`');
  assert.equal(calls[3].sql, 'DROP TABLE IF EXISTS `orders`');
  assert.equal(calls[4].sql, 'SET FOREIGN_KEY_CHECKS = 1');
...
test('restore command requires explicit destructive confirmation', async () => {
  await assert.rejects(
    runRestoreCommand({
      ...
      env: baseEnv,
      output: silentOutput(),
      async restoreBackup() { throw new Error('must not run'); },
    }),
...
test('CLI routes backup options without weakening argument validation for existing commands', async () => {
  const calls = [];
  await runCli(['backup', '--output', './safe'], {
    cwd: '/srv/yuncms',
    env: baseEnv,
    output: silentOutput(),
  ...
  assert.deepEqual(calls[0].args, ['--output', './safe']);

  await assert.rejects(
    runCli(['start', '--unexpected'], {
      env: baseEnv,
      output: silentOutput(),
      async startCommand() { return { code: 0 }; },
    }),
  ...
  const result = await runUpdateCommand({
    ...
    async collectPreflight() {
      return {
        ...
        targetVersion: '0.1.2',
        upToDate: false,
        pendingMigrations: ['0013-example'],
        databaseBytes: 1024,
    ...
    async runProcess() { processCalled = true; },
  });
...
test('successful update backs up first, installs exact target, bootstraps and probes readiness', async () => {
  ...
  try {
    const fixture = await createBackupFixture(cwd);
    const sequence = [];
    const result = await runUpdateCommand({
      args: ['--to', '0.1.2'],
      cwd,
      ...
      async createBackup() { sequence.push('backup'); return fixture; },
      async runProcess(command, args) {
        sequence.push(args.includes('bootstrap') ? 'bootstrap' : `install:${command}`);
        return { stdout: '', stderr: '', code: 0 };
      },
      async verifyRuntime() { sequence.push('probe'); return { ready: true }; },
    });

    assert.deepEqual(sequence, ['backup', 'install:npm', 'bootstrap', 'probe']);
      ...
      runUpdateCommand({
        ...
        async runProcess(_command, args) {
          ...
          if (args.includes('bootstrap')) {
            sequence.push('bootstrap-fail');
            const error = new Error('migration failed');
            error.code = 'ER_SIMULATED_DDL';
            throw error;
          }
          sequence.push(args[0] === 'ci' ? 'npm-ci' : 'npm-install');
          return { stdout: '', stderr: '', code: 0 };
        },
        async restoreBackup() { sequence.push('restore'); return fixture; },
        async verifyRuntime() { sequence.push('rollback-probe'); return { ready: true }; },
      }),
      (error) => error.code === 'ER_SIMULATED_DDL'
        && error.rollbackPerformed === true
        && error.backupPath === fixture.backupPath,
    );
