  ...
  const database = {
    async query(sql) {
      if (sql.includes('FROM yuncms_studio_settings')) {
        return [[{ public_registration_enabled: 0, public_registration_role: REGISTRATION_ROLE_ID }], []];
      }
...
test('public registration assigns only the configured normal role', async () => {
  let inserted = null;
  const database = {
    async query(sql, params = []) {
      const normalized = sql.replace(/\s+/g, ' ').trim();
      if (normalized.includes('FROM yuncms_studio_settings')) {
        return [[{ public_registration_enabled: 1, public_registration_role: REGISTRATION_ROLE_ID }], []];
      }
